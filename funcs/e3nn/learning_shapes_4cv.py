...
def train(net, train_loader, val_loader, device):
    ...
    writer = SummaryWriter(f'runs/4cv_fixed10')
...
if __name__ == '__main__':
    ...
    torch.save(train_predictions, 'train_preds_4cv_fixed10.pt')
    torch.save(train_true_labs, 'train_true_4cv_fixed10.pt')
    
    torch.save(model.state_dict(), 'e3nn_4cv_fixed10.pt')
    

    # Test the model 
    test_loader = makeLoader(test_dataset, 128)
    true_labs, predictions = test(model, test_loader, device)
    torch.save(predictions, 'y_preds_4cv_fixed10.pt')
    torch.save(true_labs, 'y_true_4cv_fixed10.pt')
