...
class Network(torch.nn.Module):
    def __init__(self) -> None:
        ...
        gate = Gate(
            ...
            "16x2o + 16x2e"  # gated tensors, num_irreps has to match with gates
        )
        # 224 -> 200
        gate2 = Gate(
            "28x0e + 28x0o", [torch.relu, torch.abs],  # scalar
            "6x0e + 6x0o + 6x0e + 6x0o", [torch.relu, torch.tanh, torch.relu, torch.tanh],  # gates (scalars)
            "12x2o + 12x2e"  # gated tensors, num_irreps has to match with gates
        )
        # 200 -> 104
        gate3 = Gate(
            "16x0e + 16x0o", [torch.relu, torch.abs],  # scalar
...
def train(net, train_loader, val_loader, device):
    ...
    writer = SummaryWriter(f'runs/4cv+_fixed10')
...
if __name__ == '__main__':
    ...
    torch.save(train_predictions, 'train_preds_4cv+_fixed10.pt')
    torch.save(train_true_labs, 'train_true_4cv+_fixed10.pt')
    torch.save(model.state_dict(), 'e3nn_4cv+_fixed10.pt')
    
    # Test the model 
    test_loader = makeLoader(test_dataset, 128)
    true_labs, predictions = test(model, test_loader, device)
    torch.save(predictions, 'y_preds_4cv+_fixed10.pt')
    torch.save(true_labs, 'y_true_4cv+_fixed10.pt')
