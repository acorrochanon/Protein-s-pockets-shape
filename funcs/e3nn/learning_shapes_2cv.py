...
class Network(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.final = Convolution(self.gate.irreps_out, self.irreps_sh, "1x0e", self.num_neighbors)
...
def train(net, train_loader, val_loader, device):
    ...
    writer = SummaryWriter(f'runs/2cv_fixed10')
...
if __name__ == '__main__':
    ...
    torch.save(train_predictions, 'train_preds_2cv_fixed10.pt')
    torch.save(train_true_labs, 'train_true_2cv_fixed10.pt')
    
    #Save the model
    torch.save(model.state_dict(), 'e3nn_2cv_fixed10.pt')

    # Test the model 
    test_loader = makeLoader(test_dataset, 128)
    true_labs, predictions = test(model, test_loader, device)
    torch.save(predictions, 'y_preds_2cv_fixed10.pt')
    torch.save(true_labs, 'y_true_2cv_fixed10.pt')
